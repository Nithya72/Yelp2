...
router.post('/', checkAuth, async (req, res, next) => {
    upload(req, res, async (err) => {
        ...
        console.log("Inside Dish Pic Updation :", req.body.profilePic, " : ", req.body.id);
        if (err) {
            ...
            data.restId = req.body.restId;
            data.id = req.body.id;

            console.log("Done! Res", data.fileName);

            kafka.make_request('res_dish_pic', data, function(err,results){   
                if (err){
